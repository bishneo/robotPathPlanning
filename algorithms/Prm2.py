...
def dijkstra_planning(sx, sy, gx, gy, ox, oy, rr, road_map, sample_x, sample_y):
    ...
    while True:
        ...
        # show graph
        if show_animation and len(closedset.keys()) % 2 == 1:
...
def prm2(start, goal, grid):
    ...
    for i in range(len(grid)):
        for j in range(len(grid[0])):
            if grid[i][j].value != 0:
                ox.append(grid[i][j].point[0])
                oy.append(grid[i][j].point[1])
    ...
    path, rx, ry = PRM_planning(sx, sy, gx, gy, ox, oy, robot_size)
